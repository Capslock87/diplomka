...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (validateForm()) {
            ...
            btn.innerHTML = `<span class="loading-spinner" style="padding:0;color:white;">${translations[window.currentLang].processingBtnText}</span>`;
            btn.disabled = true;
    ...
    function validateForm() {
        ...
        if (!isValid) {
            window.showToast(translations[window.currentLang].checkoutFormErrorToast, 'error');
        }
    ...
    function renderPreview() {
        ...
        const html = window.Cart.items.map(item => `
            <div class="preview-item">
                <img src="${item.image}" alt="${window.getLocalizedName(item)}" class="preview-img">
                <div class="preview-info">
                    <div class="preview-title">${window.getLocalizedName(item)}</div>
                    <div class="preview-meta">${translations[window.currentLang].productSize} ${item.size}</div>
                    <div class="preview-meta">${translations[window.currentLang].productQty} ${item.quantity}</div>
    ...
    document.addEventListener('languageChanged', renderPreview);
});
